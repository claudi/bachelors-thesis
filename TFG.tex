...
by the context, specifying when there might be confusion. In these situations, a
computer function may be called a~\emph{procedure}.
...
a size in number of bits and outputs an unique pointer to an address to a block
of memory allocated to fit the requested size, which is marked as reserved. This
function
...
\begin{enumerate}
    \item If a pure function is called more than once with the
        ...
        is usually called~\emph{memorization}

    \item Since pure functions have no side-effects, it is usually safe to call
        them on separate threads \cite{SussPureFunctionParallelization}.
\end{enumerate}
