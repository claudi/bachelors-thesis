...
and~\mintinline{Haskell}{b} there is a type whose elements are functions from
...
example, if we had a~\mintinline{Haskell}{factorial :: Int -> Int} function and
we wanted to evaluate it at~\mintinline{Haskell}{5 :: Int} we would write
\begin{minted}{Haskell}
factorial 5
\end{minted}
...
as~\mintinline{Haskell}{f}. If we consider a function~\mintinline{Haskell}{sum
:: Int -> Int -> Int} that adds two integers and two
elements~\mintinline{Haskell}{41 :: Int} and~\mintinline{Haskell}{19 :: Int} we
would have
\begin{minted}{Haskell}
sum       :: Int -> Int -> Int -- 1
sum 41    :: Int -> Int        -- 2
...
here
\begin{enumerate}
    \item The~\mintinline{Haskell}{sum :: Int -> Int -> Int} function takes a
        parameter of type~\mintinline{Haskell}{Int} and outputs a function of
...
double = multiply 2
\end{minted}
are also valid. Here the function~\mintinline{Haskell}{double} is defined in
terms of the function~\mintinline{Haskell}{multiply 2}, and as such the body
...
We can also define new types in a similar fashion to the union
\begin{minted}{Haskell}
data Shape = Circle Double | Polygon Int Double
\end{minted}
where~\mintinline{Haskell}{|} can be read as ``or''. Here, a member of
the~\mintinline{Haskell}{Shape} type could be either
a~\mintinline{Haskell}{Circle}, specified by the length of its radius or
a (regular)~\mintinline{Haskell}{Polygon}, specified the number of sides and its
apothem length. We could interpret this as having defined~\(\{\text{shapes}\} =
\{\text{circles}\} \cup \{\text{polygons}\}\) as our shapes of interest.
...
\end{minted}
but the~\mintinline{Haskell}{Circle} and~\mintinline{Haskell}{Polygon} words
...
different options equally, for example
\begin{minted}{Haskell}
