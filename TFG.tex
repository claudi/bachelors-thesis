...
In the~\Hask{} category, this is very similar to a functor.

This is no enough for a function on objects and morphisms to be a functor. The
functor laws must be satisfied as well. Unfortunately, Haskell does not provide
a way to express such laws, since for this it would need a way to write and
validate proofs, which would add too much complexity to the language. Haskell
assumes any typeclass implementations it encounters satisfy such properties,
which are only generally found in external documentation. Failing to satisfy
these laws could lead to buggy code and unintended behaviour.
...
\end{minted}
which are clearly equivalent to the functor laws found in~\ref{def:functor}.
