...
\begin{definition}[Category]
    ...
    set~\(\Hom(A,B)\) of \emph{morphisms} and a \emph{composition function}
    \[
        \circ\colon\Hom(A,B)\times\Hom(B,C) \longrightarrow
        \Hom(A,C)
    \]
    that satisfies
    \begin{enumerate}
        \item \emph{Associativity}: for every three
            morphisms~\(f\in\Hom(A,B)\),~\(g\in\Hom(B,C)\) and~\(h\in\Hom(C,D)\)
            we have
            \[
        ...
        \item \emph{Left and right units}: for every two
            ...
            elements~\(\id_{A}\in\Hom(A,A)\),~\(\id_{B}\in\Hom(B,B)\) such that
            for every morphism~\(f\in\Hom(A,B)\) we have
            \[
