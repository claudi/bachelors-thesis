...
We begin with an observation: functions found in computer programming, which we
call~\emph{procedures}, may not behave like functions in mathematics.

An example of this is the \texttt{malloc()} procedure found in
the~\texttt{stdlib} library in the~\texttt{C} programming language, which takes
a size in number of bits and outputs an unique pointer to an address to memory
allocated to fit the requested size. This procedure
...
\begin{definition}[Pure function]
    We say that a procedure that does \textbf{not} satisfy the above stated
    properties is~\emph{pure}.
    Conversely, we say a procedure that is not pure is~\emph{impure}.
\end{definition}

So, the~\texttt{malloc()} procedure is impure, and a procedure that outputs the
