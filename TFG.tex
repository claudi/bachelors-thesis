...
by the context, specifying when there might be confusion. In these situations,
we may refer to a computer function as a~\emph{procedure}.
...
\begin{enumerate}
    ...
    \item Since pure functions have no side-effects, it is usually safe to call
        them in parallel \cite{SussPureFunctionParallelization}.
\end{enumerate}
...
and then we will see how it can be successfully implemented into an useful
